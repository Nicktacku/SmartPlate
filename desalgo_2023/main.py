...
def open_meal_page():
    ...
    def add_meal(event=None):
        ...
        meal_query.search_meal(query, meals)
        # print(meals)
    ...
    def delete_meal():
        ...
        meals.pop(meal_listbox.get(selected_index))
        if len(selected_index) > 0:
            meal_listbox.delete(selected_index)

    query_label = tk.Label(
        meal_frame, text="Query:", font=("Helvetica", 14), bg="#d0f0c0", fg="black"
    )
    ...
    meal_listbox = tk.Listbox(
        meal_listbox_frame,
        font=("Helvetica", 14),
        bg="#eedc82",
        fg="black",
        selectbackground="#fada5e",
    ...
    def open_optimize_page():
        ...
        def start_optimization():
            ...
            print(nutriscores)
            optimize_page.after(10000, open_result_page) #open result page after 10s
        ...
        def open_result_page():
            ...
            header_label = tk.Label(result_page, text="Optimized Meal Selections using Fractional Knapsack!", font=("Helvetica", 18), bg="#d0f0c0", fg="black")
            ...
            # Create scrollbar
            scrollbar = ttk.Scrollbar(result_frame, orient="vertical", command=result_table.yview)
            scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
            result_table.configure(yscrollcommand=scrollbar.set)

            # Configure Treeview columns
            result_table['columns'] = ("meal_name", "quantity", "nutriscore", "protein", "carbohydrates", "calorie", "sugar")
            ...
            result_data = [
                ("Meal 1", "2", "A", "20g", "50g", "250", "5g"),
                ("Meal 2", "1", "B", "10g", "30g", "150", "2g"),
                ("Meal 3", "3", "C", "15g", "40g", "200", "3g"),
            ]
...
def open_calorie_page():
    ...
    options_label = tk.Label(calorie_page, text="Choose the values you wish to include in the output:", font=("Helvetica", 18), bg="#d0f0c0", fg="black")
    ...
    nutriscore_checkbox = tk.Checkbutton(calorie_page, text="Nutriscore", variable=nutriscore_var, font=("Helvetica", 12), bg="#d0f0c0", fg="black", anchor='w')
    nutriscore_checkbox.pack(pady=5, anchor='w')

    carbohydrates_checkbox = tk.Checkbutton(calorie_page, text="Carbohydrates", variable=carbohydrates_var, font=("Helvetica", 12), bg="#d0f0c0", fg="black", anchor='w')
    carbohydrates_checkbox.pack(pady=5, anchor='w')

    protein_checkbox = tk.Checkbutton(calorie_page, text="Protein", variable=protein_var, font=("Helvetica", 12), bg="#d0f0c0", fg="black", anchor='w')
    protein_checkbox.pack(pady=5, anchor='w')

    sugar_checkbox = tk.Checkbutton(calorie_page, text="Sugar", variable=sugar_var, font=("Helvetica", 12), bg="#d0f0c0", fg="black", anchor='w')
